...
// return n < m
function lessThanBigInt(n, m) {
  // n and m are big integer strings.
...
module.exports = (function() {
  ...
  var adapter = {
    ...
    registerConnection: function(connection, collections, cb) {
      ...
      var pg = require('pg');
      // Do not parse point type.
      pg.types.setTypeParser(600, 'text', function(val) {
        return val;
      });
        ...
        async.each(records, function(data, cb) {
          ...
          client.query(query.query, query.values, function __CREATE_EACH__(err, result) {
            ...
            function checkSequence(item, next) {
              ...
              if (lessThanBigInt(currentValue, sequenceValue)) {
                item.value = sequenceValue;
              }
              next();
            }
